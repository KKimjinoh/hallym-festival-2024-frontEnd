      ...
      <div className="container">
        <div className="wrapper">
          <div className="upper">
            ...
            <p>학우님들과 같은 입장의 학생으로 꾸려진 본 축제추진위원회의 안전을</p>
            <p>위해서 매뉴얼로 진행된다는 점 양해부탁드립니다.</p>
            <p>불응 시 법적조치가 진행될 수 있음을 인지해주시면 감사하겠습니다.</p>
          </div>
          {showlist ? (
              ...
              <div className="notice-list">
                {notices.map((notice, index) => (
                  <div key={index} className="notice-item">
                    <div className="title">{notice.title && <p> {notice.title}</p>}</div>
                    <div className="content">{notice.content && <p>{notice.content}</p>}</div>
                  </div>
                ))}
              </div>
              <img
